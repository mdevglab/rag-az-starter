...
//1. Run `azd env set AZURE_AISERVICES_NAMES {Name of existing OpenAI service}` // TODO multiple services.
Run `azd env set AZURE_AI_SERVICE_NAME {Name of existing OpenAI service}`
...
1. Run `azd env set AZURE_AI_CHAT_MODEL_NAME {Model name of existing chat deployment}`. Only needed if your chat model is not the default 'gpt-35-turbo'.
1. Run `azd env set AZURE_AI_CHAT_MODEL_VERSION {Version string for existing chat deployment}`. Only needed if your chat deployment model version is not the default '0125'. You definitely need to change this if you changed the model.
1. Run `azd env set AZURE_AI_CHAT_DEPLOYMENT_SKU {Name of SKU for existing chat deployment}`. Only needed if your chat deployment SKU is not the default 'Standard', like if it is 'GlobalStandard' instead.
1. Run `azd env set AZURE_AI_EMBED_DEPLOYMENT_NAME {Name of existing embedding deployment}`. Only needed if your embeddings deployment is not the default 'embedding'.
1. Run `azd env set AZURE_AI_EMBED_MODEL_NAME {Model name of existing embedding deployment}`. Only needed if your embeddings model is not the default 'text-embedding-ada-002'.
1. Run `azd env set AZURE_AI_EMBED_DIMENSIONS {Dimensions for existing embedding deployment}`. Only needed if your embeddings model is not the default 'text-embedding-ada-002'.
1. Run `azd env set AZURE_AI_EMBED_MODEL_VERSION {Version string for existing embedding deployment}`. If your embeddings deployment is one of the 'text-embedding-3' models, set this to the number 1.
